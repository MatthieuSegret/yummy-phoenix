defmodule YummyWeb.Coherence.ViewHelpers do
  @moduledoc """
  ...
  @seperator {:safe, "&nbsp; | &nbsp;"}
  @helpers YummyWeb.Router.Helpers
  ...
  * :register - customize the register link text (#{@register_link})
  ...
  def coherence_links(conn, :layout, opts) do
    ...
    if Coherence.logged_in?(conn) do
      ...
      [
        profile_link(current_user, conn, class: "navbar-item"),
        signout_link(conn, signout, class: "navbar-item")
      ]
    else
      signin_link = link(signin, to: coherence_path(@helpers, :session_path, conn, :new), class: "navbar-item")
      if Config.has_option(:registerable) && register do
        [link(register, to: coherence_path(@helpers, :registration_path, conn, :new), class: "navbar-item"), signin_link]
      else
        signin_link
      end
    end
  ...
  def signout_link(conn, text \\ @signout_link, [class: signout_class]) do
  ...
  defp profile_link(current_user, conn, opts) do
    if Config.user_schema.registerable? do
      link current_user.name, to: coherence_path(@helpers, :registration_path, conn, :show), class: opts[:class]
    else
      current_user.name
    end
  end
